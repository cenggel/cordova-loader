...
function cl (options) {
  ...
  this._options.cordovaPath = this._options.cordovaPath && path.resolve(process.env.PWD, options.cordovaPath) || null;
...
cl.prototype._addCoreFiles = function (callback) {
  ...
  this._platformsToCompile.forEach(function (platform) {
    ...
    location = path.join(self._options.cordovaPath, 'platforms', platform, wwwPath, 'cordova.js');
    self._cordovaFiles.core[platform].push(location);
    Logger.log('cordova', 'Adding ' + platform + ' Corodva file');

    location = path.join(self._options.cordovaPath, 'platforms', platform, wwwPath, 'cordova_plugins.js');
    self._cordovaFiles.core[platform].push(location);
...
cl.prototype._addPluginFiles = function (callback) {
  ...
  async.each(this._platformsToCompile, function (platform, callback) {
    ...
    var pluginJsFilePath = path.join(self._options.cordovaPath, 'platforms', platform, wwwPath, 'cordova_plugins.js');     
    ...
    fs.readFile(pluginJsFilePath, 'utf8', function (err, data) {
      ...
      plugins.forEach(function (plugin) {
        location = path.join(self._options.cordovaPath, 'platforms', platform, wwwPath, plugin.file);
...
cl.prototype._serve = function () {
  ...
  WebApp.connectHandlers.use(function(req, res, next) {
    ...
    if (/iPhone|iPad|iPod/i.test(req.headers['user-agent'])) {
      platform = 'ios';
    } else if (/Android/i.test(req.headers['user-agent']) || /Crosswalk/i.test(req.headers['user-agent'])){
      platform = 'android';
...
cl.prototype._watch = function () {
  ...
  watch.watchTree(this._options.cordovaPath + '/plugins', {ignoreDotFiles: true}, function (f, curr, prev) {
